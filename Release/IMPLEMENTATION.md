...
- Each attempt writes a distinct JSON report with expected/actual pages, bytes, source hashes, output/diagnostic path, warnings, status and a publication flag. Failed/short output remains in its staging directory for diagnosis. New destinations use atomic no-replace hardlink publication; explicitly confirmed existing destinations use atomic rename after a stale-target hash check. A filesystem without hardlinks fails safely and retains diagnostic output. The CLI refuses existing destinations; the native save chooser retains its explicit replacement question. The stale-target check is not an atomic compare-and-swap against hostile simultaneous filesystem changes.
...
ctest --test-dir build -E 'PaletteTabTest|PageTemplateDialogTest' --output-on-failure
