# Failure-path tests for the real Scriblark qualification orchestration.
...
function New-FakeOperations([string]$PrimaryFailure, [string[]]$CleanupFailures, [switch]$Noisy) {
    $global:InkQuayQualificationTestCalls = [Collections.Generic.List[string]]::new()
    $operations = [ordered]@{}
    foreach ($name in @('Preflight','PrepareSignedCopy','Install','ActivateAndVerify','CloseCleanly','UninstallAndVerify')) {
        $operationName = $name
        $operations[$name] = {
            $global:InkQuayQualificationTestCalls.Add($operationName)
...
$result = Invoke-InkQuayQualificationCore -Operations (New-FakeOperations '' @())
...
Assert-True (($global:InkQuayQualificationTestCalls -join ',') -eq 'Preflight,PrepareSignedCopy,Install,ActivateAndVerify,CloseCleanly,UninstallAndVerify,StopOwnedProcess,RemoveOwnedPackage,RemoveTrustedCertificate,RemovePersonalCertificate,RemoveTemporaryFiles') 'all qualification and cleanup steps must run in order'

$result = Invoke-InkQuayQualificationCore -Operations (New-FakeOperations 'ActivateAndVerify' @('RemoveOwnedPackage','RemovePersonalCertificate'))
...
Assert-True (-not ($global:InkQuayQualificationTestCalls -contains 'CloseCleanly')) 'later primary operations must not run after failure'
foreach ($cleanup in @('StopOwnedProcess','RemoveOwnedPackage','RemoveTrustedCertificate','RemovePersonalCertificate','RemoveTemporaryFiles')) {
    Assert-True ($global:InkQuayQualificationTestCalls -contains $cleanup) "cleanup operation $cleanup must still run"
}

$result = Invoke-InkQuayQualificationCore -Operations (New-FakeOperations '' @('RemoveTrustedCertificate'))
...
$result = Invoke-InkQuayQualificationCore -Operations (New-FakeOperations 'Preflight' @())
Assert-True (-not $result.installation_qualification_passed) 'preexisting-install/preflight failure must fail qualification'
Assert-True (($global:InkQuayQualificationTestCalls -join ',') -eq 'Preflight,StopOwnedProcess,RemoveOwnedPackage,RemoveTrustedCertificate,RemovePersonalCertificate,RemoveTemporaryFiles') 'preflight failure must skip mutation and still execute safe cleanup adapters'

$result = @(Invoke-InkQuayQualificationCore -Operations (New-FakeOperations '' @() -Noisy))
...
$result = @(Invoke-InkQuayQualificationCore -Operations (New-FakeOperations 'ActivateAndVerify' @('RemovePersonalCertificate') -Noisy))
...
$insidePackage = Test-PathInside -Candidate (Join-Path $packageRoot 'Scriblark.exe') -Root $packageRoot
...
$recordFixture = [pscustomobject]@{ payload = [pscustomobject]@{ 'Scriblark.exe' = [pscustomobject]@{ bytes=1; sha256=('a' * 64) } } }
Assert-True ((Get-RecordPayloadEntry $recordFixture 'Scriblark.exe').bytes -eq 1) 'slash-qualified payload property must resolve exactly'
$exclusiveDirectory = Join-Path ([IO.Path]::GetTempPath()) ('inkquay-ps-test-' + [guid]::NewGuid().ToString('N'))
...
Assert-True ($null -ne ('InkQuayQualification.NativePackageProbe' -as [type])) 'GetPackageFullName helper types must compile'

Remove-Variable InkQuayQualificationTestCalls -Scope Global
