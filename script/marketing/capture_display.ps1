...
function Add-ScriblarkMarketingConsumerDisplayTypes {
    if('ScriblarkMarketingConsumerDisplay.DisplayModes' -as [type]){return}
    Add-Type -TypeDefinition @'
using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.Runtime.InteropServices;
namespace ScriblarkMarketingConsumerDisplay {
...
function Convert-ScriblarkMarketingConsumerMode($Mode){
...
function Get-ScriblarkMarketingConsumerModeChoice($Modes){
...
function Get-ScriblarkMarketingConsumerDisplayState([string]$Device){
    Add-ScriblarkMarketingConsumerDisplayTypes
    if(-not $Device){$Device=[ScriblarkMarketingConsumerDisplay.DisplayModes]::Primary()}
    return @{device=$Device;current=[ScriblarkMarketingConsumerDisplay.DisplayModes]::Current($Device);modes=[ScriblarkMarketingConsumerDisplay.DisplayModes]::Supported($Device)}
}
function Set-ScriblarkMarketingConsumerDisplayMode([string]$Device,$Mode,[int]$Flags){
    return [ScriblarkMarketingConsumerDisplay.DisplayModes]::Change($Device,$Mode,$Flags)
}

function Start-ScriblarkMarketingConsumerDisplay($State){
    $snapshot=Get-ScriblarkMarketingConsumerDisplayState
    $State.displayOriginalMode=$snapshot.current;$State.displayDevice=$snapshot.device
    $State.displayEvidence=[ordered]@{purpose='actual native display only';device=$snapshot.device;before=(Convert-ScriblarkMarketingConsumerMode $snapshot.current);
        supported_modes=@($snapshot.modes|ForEach-Object {Convert-ScriblarkMarketingConsumerMode $_});selected=$null;test_result=$null;apply_result=$null;after=$null;
    ...
    if($snapshot.current.dmPelsWidth -ge 1920 -and $snapshot.current.dmPelsHeight -ge 1080 -and $snapshot.current.dmBitsPerPel -eq 32){
        $State.displayEvidence.after=Convert-ScriblarkMarketingConsumerMode $snapshot.current;return
    }
    $selected=Get-ScriblarkMarketingConsumerModeChoice $snapshot.modes;$State.displayEvidence.selected=Convert-ScriblarkMarketingConsumerMode $selected
    $test=Set-ScriblarkMarketingConsumerDisplayMode $snapshot.device $selected 2;$State.displayEvidence.test_result=$test
    ...
    $apply=Set-ScriblarkMarketingConsumerDisplayMode $snapshot.device $selected 0;$State.displayEvidence.apply_result=$apply
    if($apply -ne 0){throw "Native dynamic display change failed ($apply)."}
    $deadline=[DateTime]::UtcNow.AddSeconds(5)
    do{
        $after=(Get-ScriblarkMarketingConsumerDisplayState $snapshot.device).current
    ...
    $State.displayEvidence.after=Convert-ScriblarkMarketingConsumerMode $after
...
function Restore-ScriblarkMarketingConsumerDisplay($State){
    if(-not $State.displayOriginalMode){return}
    if($State.displayRestoreRequired){
        $result=Set-ScriblarkMarketingConsumerDisplayMode $State.displayDevice $State.displayOriginalMode 0;$State.displayEvidence.restore_result=$result
        if($result -ne 0){throw "Native display restoration failed ($result)."}
    }
    $deadline=[DateTime]::UtcNow.AddSeconds(5)
    do{
        $observed=Convert-ScriblarkMarketingConsumerMode (Get-ScriblarkMarketingConsumerDisplayState $State.displayDevice).current
